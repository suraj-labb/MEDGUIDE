...
DATA = "/content/drive/MyDrive/MED0.2/data/indian_medicines.csv"
...
def load_data():
    df = pd.read_csv(DATA)
    df["clean_name"] = df["name"].fillna("").str.lower()
    return df
...
def strength(name):
    m = re.search(r'(\d+(?:\.\d+)?)\s*(mg|ml|mcg|g)\b', name.lower())
    if m:
        return m.group(1) + m.group(2)
    m = re.search(r'(\d+(?:\.\d+)?)\s*(?=tablet|capsule|tab\b)', name.lower())
        ...
        with st.spinner("Reading prescription..."):
            messages = [{
                ...
                "content": [
                    ...
                    {"type": "text", "text":
                     'Read ONLY the medicine names and strengths. '
            ...
            medicines = re.findall(r'"drug"\s*:\s*"([^"]+)"', raw)
...
st.caption("MEDGUIDE is an educational prototype and not a substitute for professional medical advice.")
